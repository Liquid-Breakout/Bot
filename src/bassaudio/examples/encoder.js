...
if (args.length < 3) {
    console.log("Usage : node encoder.js [*.mp3] [complete_cast_url] [complete_ID]\nSee http://www.un4seen.com/doc/#bassenc/BASS_Encode_CastInit.html");
    process.exit();
}
...
var mixerFlag = basslib.BASS_Encode_Startflags.BASS_ENCODE_CAST_NOLIMIT |
...
// BASS_Encode_Start
var _encoder = basslib.BASS_Encode_MP3_Start(mixer, lamestr, basslib.BASS_Encode_Startflags.BASS_ENCODE_NOHEAD);
if (_encoder === 0) {
    console.log(`${chalk.bgRed.white.bold("error at BASS_Encode_MP3_Start: ")} ${basslib.BASS_ErrorGetCode()}`);
    process.exit();
}
else {
...
setTimeout(() => {
    var isCast = basslib.BASS_Encode_CastInit(_encoder, URL, ID, "audio/mpeg", "Bassaudio encoder test", "", "", null, null, BIT_RATE, true);
    if (!isCast) {
        var errorCode = basslib.BASS_ErrorGetCode();
        console.log(chalk.bgRed.white.bold("cast ERROR reason:" +
            errorCode +
            ":" +
