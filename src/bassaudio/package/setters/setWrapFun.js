...
function setWrapFun(bass) {
    ...
    bass.getCodeMessageFrom = (code, flagType) => {
        for (let flagMessage in flagType)
            if (flagType[flagMessage] === code)
                return flagMessage;
    };
    bass.LoadAllPlugins = () => {
        let result = [];
        for (let i in bass.addonsEncodings) {
            result.push(bass.BASS_PluginLoad(path.join(bass.basePath, bass.addonsEncodings[i])), bass.addonsEncodings[i]);
        }
        return result;
    };
    bass.EnableAllAvailable = (value) => {
        for (let i in bass.enableFuns)
            bass[bass.enableFuns[i]](value);
    };
    bass.AreAllAvailableEnabled = (value) => {
        const notEnabled = [];
        for (let libname in bass.libFiles)
            if (!bass.libFiles[libname].isEnabled())
                notEnabled.push(libname);
    ...
    bass.WRAP_ChannelGetTags = function (handle, tags) {
        ...
        if (tags == this.BASS_ChannelGetTagtypes.BASS_TAG_ID3 ||
    ...
    bass.WRAP_ChannelSetSync = function (handle, type, param, callback) {
        return this.libFiles["bass"].rawFunc("BASS_ChannelSetSync", handle, type, param, ffi.Callback("void", ["int", "int", "int", "pointer"], callback), null);
    };
    bass.WRAP_Mixer_ChannelSetSync = function (handle, type, param, callback) {
        return this.libFiles["mix"].rawFunc("BASS_Mixer_ChannelSetSync", handle, type, param, ffi.Callback("void", ["int", "int", "int", "pointer"], callback), null);
    };
    bass.WRAP_Encode_SetNotify = function (handle, callback) {
        return this.libFiles["enc"].rawFunc("BASS_Encode_SetNotify", handle, ffi.Callback("void", ["int", "int", ref.types.void], callback), null);
    };
    bass.WRAP_RecordStart = function (freq, chans, flags, callback) {
        if (callback) {
            return this.libFiles["bass"].rawFunc("BASS_RecordStart", freq, chans, flags, ffi.Callback("bool", ["int", ref.types.void, "int", ref.types.void], callback), null);
        }
        return this.libFiles["bass"].rawFunc("BASS_RecordStart", freq, chans, flags, null, null);
    };
    bass.getVolume = function (channel) {
        ...
        try {
            ...
            this.BASS_ChannelGetAttribute(channel, this.BASS_ChannelAttributes.BASS_ATTRIB_VOL, volume);
    ...
    bass.setVolume = function (channel, newVolume) {
        return this.BASS_ChannelSetAttribute(channel, this.BASS_ChannelAttributes.BASS_ATTRIB_VOL, newVolume / 100);
    };
    bass.getPosition = function (channel) {
        return this.BASS_ChannelBytes2Seconds(channel, this.BASS_ChannelGetPosition(channel, 0));
    };
    bass.getDuration = function (channel) {
        return this.BASS_ChannelBytes2Seconds(channel, this.BASS_ChannelGetLength(channel, 0));
    };
    ...
    bass.getRecordDevices = function () {
        ...
        while (this.BASS_RecordGetDeviceInfo(i, info.ref())) {
            if (info.flags & this.BASS_DEVICEINFOflags.BASS_DEVICE_ENABLED &&
                (info.flags & this.BASS_DEVICEINFOflags.BASS_DEVICE_TYPE_MASK) ==
                    this.BASS_DEVICEINFOflags.BASS_DEVICE_TYPE_MICROPHONE) {
                // micdev = i;
                var o = new Object();
