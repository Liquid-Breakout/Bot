...
const fs = require("fs");
const libFile = require("./libFile.js");
const enableLibInt = require("./enableLibInt.js");
const getPlatformDependencies = require("./getPlatformDependencies.js");
...
var libDeclarations = [];
fs.readdirSync(path.join(__dirname, "libDeclarations")).forEach(function (file) {
    if (file.indexOf(".js") > -1)
...
function Bass(options) {
    ...
    this.FfiFunDeclarationIndex = require("./FfiFunDeclarationIndex.js");
    if (options.generatedFfiFunDeclaration) {
        for (let libname in options.generatedFfiFunDeclaration) {
            if (!this.libFiles[libname]) {
                this.libFiles[libname] = new libFile(libname, options.generatedFfiFunDeclaration[libname].path);
                this.libFiles[libname].setPath();
            }
            this.FfiFunDeclarationIndex.add(libname, options.generatedFfiFunDeclaration[libname].ffiFunDeclaration);
        }
    }
    ...
    for (let i in libDeclarations) {
        if (libDeclarations[i])
            this.FfiFunDeclarationIndex.add(libDeclarations[i].key, libDeclarations[i].getFfiFunDeclarations(this));
    }
