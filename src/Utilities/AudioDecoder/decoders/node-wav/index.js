...
var data_decoders = {
  ...
  pcm32f: function pcm32f(buffer, offset, output, channels, samples) {
    ...
    var pos = 0;
    for (var i = 0; i < samples; ++i) {
      for (var ch = 0; ch < channels; ++ch) output[ch][i] = input[pos++];
    }
  },
...
function decode(buffer) {
  ...
  function string(len) {
    var str = '';
    for (var i = 0; i < len; ++i) str += String.fromCharCode(u8());
    return str;
  }
  ...
  while (pos < end) {
    ...
    switch (type) {
      ...
      case 'data':
        ...
        var channelData = [];
        for (var ch = 0; ch < channels; ++ch) channelData[ch] = new Float32Array(samples);
